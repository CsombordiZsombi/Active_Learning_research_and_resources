# Soruce: https://www.kaggle.com/code/akhileshravi/active-learning-tutorial/notebook
...
origdata = pd.read_csv(".../SVM iris species/input/iris/Iris.csv")
origdata[:10]
...
plt.scatter(X[k1][versicolor], X[k2][versicolor], c='r')  ##versicolor red
plt.scatter(X[k1][virginica], X[k2][virginica], c='c')    ##virinica cyan 

plt.show()

X1 = X[y != 0]  ##No setosas in X1
y1 = y[y != 0]
...
plt.plot(lx0, ly0, c='m')  ##magenta line, original
...
def plot_svm(clf, train_indexes, unknown_indexes, new_index = False, title = False, name = False):  ##Show svc
    ...
    lx = [xmin + stepx * i for i in range(100)] ##line x
    ly = [-(a*lx[i] + c)/b for i in range(100)] ##line y
    ...
    plt.plot(lx, ly, c='m')          ##magenta line current 
    plt.plot(lx0, ly0, '--', c='g')  ##green -- line original
